...
**Atualização 3:** o perfil (hub) ganhou mais campos de identidade — o pedido original de portfólio incluía "capa, bio, cores e informações de contato", isso ainda faltava. Agora `PortfolioProfile` também tem `capaUrl` (banner do hub, mesmo padrão 2-passos de upload da foto — novo `POST /portfolio-profile/cover-upload-url`), `bio` (texto livre) e `links[]` (contato/redes livres, tipo `{ id, rotulo, url }`, sem lista fixa de plataformas — o owner nomeia como quiser: "Site", "WhatsApp", etc). Os três só entram em `PATCH /portfolio-profile` (a capa via `capaUrl` depois do upload, `bio`/`links` direto). Além disso, `PortfolioItem` ganhou `destaque: boolean` (o owner marca manualmente os melhores trabalhos, editável em `PATCH /portfolios/:id/videos/:videoId`) — aparece nas duas rotas públicas (`GET /public/portfolios/:linkPublico` e, dentro de cada álbum, no hub) só como um sinal visual (badge "Destaque"), não muda a ordenação. Cores por portfólio/template ainda não entraram — só a cor de destaque única da marca da agência que já existia.
...
Toda a tela (`/portfolios`, `/portfolios/:id`, `/p/:link`, `/portfolio/:linkHub`) já está funcional em modo demo (`?demo=1` / "Entrar como demo" no login) — dá pra ver o fluxo completo: criar categorias, subir foto de perfil, criar portfólio já numa categoria, selecionar vídeo existente, upload de vídeo/foto dedicado, capa do álbum, reordenar, remover, copiar/abrir o link do hub e de cada álbum, e navegar do hub pro álbum. Inclui também o que entrou na atualização 3: editar capa/bio/links do hub (botão "Editar perfil" na lista de portfólios) e marcar/desmarcar um item como destaque (estrela no card do item, dentro de um álbum). Assim que os endpoints acima existirem, é só o backend responder — não deve precisar de nenhuma mudança do meu lado.
